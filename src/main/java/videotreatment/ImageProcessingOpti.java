...
//        System.out.println("Loop duration : " + (loopTime - startingTime));
    ...
    void extractHistogram(BufferedImage image, List<HoughCircles.Eyes> listEyes) {
        ...
        if (!firstUes.isEmpty() && !secondUes.isEmpty()) {
            ...
            float secondUeMedian = secondUes.get(secondUes.size() / 2);
            float secondSaturationMedian = secondSaturation.get(secondUes.size() / 2);
            float secondBrightnessMedian = secondBrightness.get(secondUes.size() / 2);
